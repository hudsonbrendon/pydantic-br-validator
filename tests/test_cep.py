...
def cep_digits():
    ceps = [re.sub("[^0-9]", "", fake.cep()) for _ in range(TOTAL_CEP)]
    return ceps


def cep_mask():
    ceps = [fake.cep() for _ in range(TOTAL_CEP)]
    return ceps


def cep_mixed():
    ceps = [fake.cep() for _ in range(int(TOTAL_CEP / 2))]
    ceps += [re.sub("[^0-9]", "", fake.cep()) for _ in range(int(TOTAL_CEP / 2))]
    return ceps
